...
def find_shortest_path_every_neighborhood(self, end: str, visited: set[str]) -> list[str]:
    ...
    # Ideally, this should never happen but this is to prevent a python TA error
    return []
...
def find_all_possible_paths(self, end: str, visited: set[str]) -> list[list[Link]]:
    ...
    # we have the start neighborhood and end neighborhood
    # we want to check every possible path to reach the endpoint, taking into account that we must pass every node
    # to compute the first route, we would call the helper function calculate_path_distance()
    ...
    for link in list(self.links.values()):  
        path_so_far = [link]  
        if link.get_other_endpoint(self) not in visited:  #get_other_endpoint is a node function that we need to implement
...
def find_all_possible_paths(self, start: str, end: str, visited: set[str]) -> tuple[list[float], list[list[str]]]:
    start_node = self._nodes[start]
    return start_node.find_paths(end, set())

# this method is called in the NODE section of find_shortest_path_every_neighborhood
def compute_path_score(path: list[str]) -> float:
    """"Helper method 2 - """
