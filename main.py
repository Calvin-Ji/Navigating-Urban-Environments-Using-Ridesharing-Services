...
def run_get_all_neighbourhoods() -> set[str]:
    """
    Returns a set of all the neighborhoods in the dataset
    """
    data = read_data.read_csv("data/My Uber Drives - 2016.csv")
    set_so_far = set()
    for keys in data:
        set_so_far.add(keys[1])
        set_so_far.add(keys[2])
    return set_so_far
...
if __name__ == '__main__':
    ...
    while start_loop:
        runner()
        start_loop = bool(input("Continue? True/False: "))
        if str(start_loop).lower() == 'true':
            start_loop = True
        else:
            start_loop = False
